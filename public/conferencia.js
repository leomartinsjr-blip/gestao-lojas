...
(function() {
  ...
  (function() {
    ...
    // Sincroniza loja com seletor principal do CMV Itens ao abrir
    $('cmvVendedorBtn').addEventListener('click', () => {
      $('vvBoard').value = $('cmvBoard').value;
      modal.style.display = '';
    });
    $('vvCloseBtn').addEventListener('click', () => { modal.style.display = 'none'; });
    modal.addEventListener('click', e => { if (e.target === modal) modal.style.display = 'none'; });

    $('vvBuscarBtn').addEventListener('click', buscarVendedor);
    ...
    async function buscarVendedor() {
      ...
      body.innerHTML = '<div style="color:var(--cf-muted);text-align:center;padding:40px"><span class="spinner"></span> Buscando…</div>';
      ...
      try {
        ...
        body.innerHTML = `<div style="color:var(--cf-alert);text-align:center;padding:40px">⚠ ${e.message}</div>`;
      }
    }
    ...
    function renderVendedor(data) {
      ...
      if (!data.rows || !data.rows.length) {
        body.innerHTML = '<div style="color:var(--cf-muted);text-align:center;padding:40px">Nenhuma venda encontrada.</div>';
        return;
      }

      // Agrupa para exibição com cabeçalhos de loja e vendedor
      let html = `<table class="cf-tbl" style="width:100%">
        <thead><tr>
          <th style="width:90px">Loja</th>
          <th style="width:140px">Vendedor</th>
          <th style="width:90px">Marca</th>
          <th style="width:65px">Código</th>
          <th>Descrição</th>
          <th class="num" style="width:45px">Qtd</th>
          <th class="num" style="width:110px">Valor Líq.</th>
        </tr></thead><tbody>`;
      ...
      for (const row of data.rows) {
        ...
        if (isNewLoja) {
          html += `<tr style="background:var(--cf-card2)">
            <td colspan="7" style="font-weight:800;font-size:12px;color:var(--cf-primary);padding:8px 10px;letter-spacing:.4px;text-transform:uppercase">${row.loja}</td>
          </tr>`;
          prevLoja = row.loja;
          prevVend = null;
        }
        if (isNewVend) {
          html += `<tr style="background:#ffffff08">
            <td></td>
            <td colspan="6" style="font-weight:700;font-size:12px;color:var(--cf-text);padding:6px 10px">
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:5px;vertical-align:middle"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
              ${row.vendedor}
            </td>
          </tr>`;
        ...
        html += `<tr>
          <td></td>
          <td></td>
          <td style="font-size:11px;color:var(--cf-muted)">${row.marca}</td>
          <td style="font-size:11px;color:var(--cf-muted)">${row.cod||'—'}</td>
          <td style="font-size:12px;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${row.desc}">${row.desc}</td>
          <td class="num" style="font-size:11px">${row.qty}</td>
          <td class="num" style="font-weight:700">${fmtR(row.venda_total)}</td>
        </tr>`;
      }
    ...
    $('vvXlsBtn').addEventListener('click', () => {
      ...
      for (const r of _vvData.rows) {
        lines.push([r.loja, r.vendedor, r.marca, r.cod, r.desc, r.qty, String(r.venda_total).replace('.',',')].join('\t'));
      }
      const blob = new Blob([lines.join('\n')], { type: 'text/tab-separated-values;charset=utf-8' });
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
